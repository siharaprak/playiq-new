...
export default async function Module8OverviewPage() {
      ...
      <section className="p-6 rounded-xl border mb-8" style={{ background: 'var(--space-card)', borderColor: 'var(--glass-border)' }}>
        ...
        <div className="flex flex-col gap-3">
          {MODULE_NODES.map((node, idx) => {
            ...
            if (mastered) {
              return (
                <Link key={node.id} href={`/student/modules/8/nodes/${node.id}/lesson`} className="p-4 rounded-lg flex items-center gap-3 transition-all group hover:bg-[rgba(57,255,20,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-green)' }}>
