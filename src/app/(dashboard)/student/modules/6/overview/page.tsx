    ...
    <div className="flex flex-col min-h-screen px-6 py-12 max-w-4xl mx-auto">
      ...
      <header className="mb-10">
        ...
        <p className="text-lg mt-3 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
          Stop relying on rereading and build a Mistake Bank to measure memory and correction.
        </p>
      </header>
      ...
      <ModulePdfDownload moduleNumber={6} title="Module 6 Student Guide: Self-Testing and Mistake Bank" className="mb-8" />
      ...
      <section className="p-6 rounded-xl border mb-8" style={{ background: 'var(--space-card)', borderColor: 'var(--glass-border)' }}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="p-4 rounded-lg border flex flex-col md:flex-row md:items-center justify-between gap-4" style={{ borderColor: quizPassed ? 'var(--neon-green)' : quizUnlocked ? 'var(--neon-yellow)' : 'var(--glass-border)' }}>
            ...
            {quizUnlocked ? (
              <Link href="/student/modules/6/quiz" className="px-4 py-2 rounded text-xs font-bold uppercase tracking-wider text-center transition-all" style={{ background: 'var(--neon-yellow)', color: '#000' }}>
          ...
          <div className="p-4 rounded-lg border flex flex-col md:flex-row md:items-center justify-between gap-4" style={{ borderColor: bossBattlePassed ? 'var(--neon-green)' : bossBattleUnlocked ? 'var(--neon-red)' : 'var(--glass-border)' }}>
            ...
            {bossBattleUnlocked ? (
              <Link href="/student/modules/6/boss-battle" className="px-4 py-2 rounded text-xs font-bold uppercase tracking-wider text-center transition-all" style={{ background: 'var(--neon-red)', color: '#fff' }}>
      ...
      <section className="p-6 rounded-xl border mb-8" style={{ background: 'var(--space-card)', borderColor: artifactsUnlocked ? 'var(--neon-cyan)' : 'var(--glass-border)' }}>
        ...
        {artifactsUnlocked ? (
          <Link href="/student/modules/6/proof-artifacts" className="inline-block px-4 py-2 rounded text-xs font-bold uppercase tracking-wider transition-all" style={{ background: 'var(--neon-cyan)', color: '#000' }}>
      ...
      <section className="mt-4">
        <ModuleFeedbackForm moduleId={MODULES.MODULE_6_ID} initialFeedback={existingFeedback} />
      </section>
    </div>
  );
}
