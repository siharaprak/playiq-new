...
export default async function Module6OverviewPage() {
  ...
  const { data: progressData } = await supabase
    ...
    .eq('module_id', MODULES.MODULE_6_ID);
...
<div className="mb-4 text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--neon-cyan)' }}>
      ...
      <section className="p-6 rounded-xl border mb-8" style={{ background: 'var(--space-card)', borderColor: 'var(--glass-border)' }}>
        ...
        <div className="flex flex-col gap-3">
          {MODULE_NODES.map((node, idx) => {
            ...
            if (mastered) {
              return (
                <Link key={node.id} href={`/student/modules/6/nodes/${node.id}/lesson`} className="p-4 rounded-lg flex items-center gap-3 transition-all group hover:bg-[rgba(57,255,20,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-green)' }}>
            ...
            if (isNext || !locked) {
              return (
                <Link key={node.id} href={`/student/modules/6/nodes/${node.id}/lesson`}
      ...
      <section className="p-6 rounded-xl border" style={{ background: 'var(--space-card)', borderColor: 'var(--neon-purple)' }}>
        ...
        <div className="flex flex-col gap-3">
          {quizUnlocked ? (
            <Link href="/student/modules/6/quiz" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(0,200,255,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-cyan)' }}>
              <span className="text-sm font-mono text-[var(--text-primary)] group-hover:text-[var(--neon-cyan)] transition-colors">Module Quiz — Module 6 Assessment Quiz</span>
              {quizPassed ? (
          ...
          {bossBattleUnlocked ? (
            <Link href="/student/modules/6/boss-battle" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(123,79,206,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-purple)' }}>
              <span className="text-sm font-mono text-[var(--text-primary)] group-hover:text-[var(--neon-purple)] transition-colors">Boss Battle — Module 6 Challenge</span>
              {bossBattlePassed ? (
          ...
          {artifactsUnlocked ? (
            <Link href="/student/modules/6/proof-artifacts" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(123,79,206,0.05)]" style={{ background: 'transparent', border: '1px solid #7b4fce' }}>
