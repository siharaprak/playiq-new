...
export default async function NodeLessonPage({ params }: { params: Promise<{ nodeId: string }> }) {
  ...
  await enforceNodeGating(nodeId, 'lesson', 6);

  const lessonData = module6Nodes[nodeId];
  ...
  return (
    <div className="flex flex-col min-h-screen px-6 py-12 max-w-3xl mx-auto">
      ...
      <Link href="/student/modules/6/overview" className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wider mb-8 transition-colors group" style={{ color: 'var(--text-secondary)' }}>
...
<div className="mb-4 text-sm text-muted-foreground uppercase tracking-wider">
        Module 6 · Node {nodeId} · Lesson Phase
      </div>
