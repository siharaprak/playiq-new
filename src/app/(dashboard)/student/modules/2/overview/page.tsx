...
<div className="mb-4 text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--neon-cyan)' }}>
      ...
      <section className="p-6 rounded-xl border mb-8" style={{ background: 'var(--space-card)', borderColor: 'var(--neon-cyan)' }}>
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
          <div>
            <p className="font-bold mb-2" style={{ color: 'var(--neon-green)' }}>What will give you an unfair advantage:</p>
            <ul className="space-y-1 list-none">
              {['Using AI to draft, iterate, and sharpen your own thinking', 'Applying the Truth Filter to never be misled', 'Protecting your focus like your most valuable asset', 'Taking the highest path in every choice'].map(item => (
                <li key={item} className="flex items-center gap-2"><span style={{ color: 'var(--neon-cyan)' }}>✓</span> {item}</li>
              ))}
            </ul>
          </div>
          <div>
            <p className="font-bold mb-2 text-red-500">What will make you dependent and passive:</p>
            <ul className="space-y-1 list-none">
              {['Blindly trusting whatever an AI says', 'Letting algorithms dictate where your time goes', 'Using technology to escape effort instead of amplifying it', 'Sacrificing long-term mastery for short-term ease'].map(item => (
      ...
      <section className="p-6 rounded-xl border" style={{ background: 'var(--space-card)', borderColor: 'var(--neon-purple)' }}>
        ...
        <div className="flex flex-col gap-3">
          {quizUnlocked ? (
            <Link href="/student/modules/2/quiz" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(0,200,255,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-cyan)' }}>
              <span className="text-sm font-mono text-[var(--text-primary)] group-hover:text-[var(--neon-cyan)] transition-colors">Module Quiz — Digital Smarts & Human Responsibility Quiz</span>
              {quizPassed ? (
          ...
          {bossBattleUnlocked ? (
            <Link href="/student/modules/2/boss-battle" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(123,79,206,0.05)]" style={{ background: 'transparent', border: '1px solid var(--neon-purple)' }}>
              <span className="text-sm font-mono text-[var(--text-primary)] group-hover:text-[var(--neon-purple)] transition-colors">Boss Battle — Module 2 Challenge</span>
              {bossBattlePassed ? (
          ...
          {artifactsUnlocked ? (
            <Link href="/student/modules/2/proof-artifacts" className="p-4 rounded-lg flex items-center justify-between transition-all group hover:bg-[rgba(123,79,206,0.05)]" style={{ background: 'transparent', border: '1px solid #7b4fce' }}>
