  ...
  return (
    <div className="flex flex-col min-h-screen px-6 py-12 max-w-3xl mx-auto">
      ...
      <div className="bg-slate-800/60 p-6 rounded-xl border border-slate-700/50 mb-8 backdrop-blur-md">
        <h3 className="text-indigo-400 font-bold uppercase tracking-widest text-xs mb-3">MISSION BRIEFING</h3>
      ...
      <div className="space-y-6 mb-12">
        {lessonData.activity.scenarios.map((scenario, i) => (
          <div key={i} className="bg-slate-900/80 p-5 border border-[#ff00ff]/30 rounded-lg shadow-lg">
             <p className="text-white font-mono text-sm mb-4 tracking-wide">&gt; {scenario}</p>
             <input type="text" placeholder="AWAITING INPUT..." className="neon-input w-full bg-black/50 border border-slate-700 focus:border-[#00f2ff] rounded p-3 text-white text-sm font-mono tracking-widest placeholder:opacity-50 outline-none" />
          </div>
        ))}
          ...
          <div className="mt-12 pt-12 border-t border-slate-800">
            ...
            <div className="space-y-6">
              {lessonData.activity.reflection.map((prompt, i) => (
                <div key={`ref-${i}`} className="bg-indigo-900/20 p-5 border border-indigo-500/40 rounded-lg">
                   <p className="text-indigo-200 font-bold mb-3">{prompt}</p>
                   <textarea placeholder="Synthesizing..." className="neon-input w-full bg-black/50 border border-slate-700 focus:border-[#00f2ff] rounded p-3 text-white text-sm font-mono h-24 outline-none" />
                </div>
              ))}
            </div>
          </div>
