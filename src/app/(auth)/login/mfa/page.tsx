...
export default function MfaVerificationPage() {
  ...
  useEffect(() => {
    // Give the browser a moment to process auth cookies from the login redirect
    const timer = setTimeout(async () => {
      try {
        // The browser client reads session from cookies automatically
        const { data: { user } } = await supabase.auth.getUser();
        
        if (!user) {
          // No session at all — go back to login
          router.replace('/login');
          return;
        }

        // Verify this user actually needs MFA
        ...
        if (mfaData && mfaData.currentLevel === 'aal2') {
          ...
          const { data: profile } = await supabase
            ...
            .eq('id', user.id)
            .single();
          router.replace(`/${profile?.role || 'parent'}/home`);
          return;
        }

        setSessionReady(true);
      } catch (err) {
        console.error('MFA session init error:', err);
        router.replace('/login');
      } finally {
        setLoading(false);
      }
    }, 300); // Small delay for cookie propagation
  ...
  if (loading) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-space-hero star-field">
        <Loader2 className="w-8 h-8 text-[#00c8ff] animate-spin" />
      </main>
    );
  }
