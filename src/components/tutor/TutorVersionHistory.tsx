...
import type { TutorVersion } from '@/lib/tutor/types';

interface TutorVersionHistoryProps {
  versions: TutorVersion[];
  currentVersionId: string | null;
}

/**
 * TutorVersionHistory — Compact list of tutor versions (newest first).
 * Current version is highlighted. Clicking a version expands detail preview.
 */
export default function TutorVersionHistory({
  versions,
  currentVersionId,
}: TutorVersionHistoryProps) {
              ...
              <div className="px-4 pb-4 pt-1 border-t border-slate-800/50 space-y-3">
                ...
                <div>
                  ...
                  <p className="font-mono text-xs text-slate-400 leading-relaxed bg-black/40 rounded p-3 border border-slate-800/50 line-clamp-4 whitespace-pre-wrap">
                    {version.instructions?.instruction_set || '(empty)'}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4 font-mono text-[10px] text-slate-500">
                  <span>
                    Rules: {version.instructions?.rules?.length ?? 0}
                  </span>
                  <span>
                    Knowledge Files: {version.knowledge_file_ids?.length ?? 0}
                  </span>
                  <span>
