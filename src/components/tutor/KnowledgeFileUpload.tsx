...
/**
 * KnowledgeFileUpload — Drag-and-drop file upload for tutor knowledge files.
 * Uploads to Supabase Storage then registers DB record via server action.
 */
export default function KnowledgeFileUpload({
  ...
  // ── Simulated upload progress (matches ArtifactUploadZone pattern) ──
  useEffect(() => {
  ...
  const uploadFile = async (file: globalThis.File) => {
    ...
    try {
      ...
      setUploadProgress(0);
      // Reset hidden input so the same file can be re-selected
  ...
  const handleDelete = async (knowledgeFile: KnowledgeFile) => {
    ...
    try {
      ...
      await deleteKnowledgeFile(knowledgeFile.id, knowledgeFile.file_url || '');
