# Runbook: Tutor Build Issues Support Workflow

This document details the support workflows for resolving tutor build issues during the PlayIQ beta phase. This runbook is an extension of the support operations system, aligning directly with the `support_issues` database table schema.

---

## 1. Scope & Category

*   **`tutor_build`**: Category for support issues related to creating, configuring, building, testing, or completing a custom Guided AI tutor (Module 9 curriculum).
...
*   **AI Code and Instruction Privacy Rules**:
    *   **never expose raw tutor instructions** in tickets, responses, or support notes.
    *   **never expose raw prompts or raw responses** of the tutor to parents or unauthorized users.
...
### Case A: Tutor Creation Errors
1.  **Validate Config Constraints**: Check if the tutor setup violates configuration constraints (e.g., empty prompt template, name containing disallowed words).
